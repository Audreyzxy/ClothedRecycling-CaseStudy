...
using BusinessObjects; /*It was already added in previous step.*/
using System.Data;
...
namespace DataAccessLayer
{
    public class DAL
    {
        ...
        public DAL()
        {
            ConnectionString = ConfigurationManager.ConnectionStrings["ClothesRecyclingSlnDB"].ConnectionString;
        }
        ...
        public int AllocateRecipientDAL(string[] strArrUpdate)
        {
            int output = 0;
            SqlConnection connection = new SqlConnection(ConnectionString);
            string sql = string.Format(@"Update[Donation] set 
RecipientID='{0}', 
status='{1}', 
CharityName='{2}'
where DonationId='{3}'",
